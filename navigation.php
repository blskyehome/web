<?php require_once ("public/header.php"); ?>
<link rel="stylesheet" href="assets/css/public.css">
</head>

<body data-type="widgets">
    <script src="assets/js/theme.js"></script>
    <div class="am-g tpl-g">
        <?php require_once ("public/main.php"); ?>
        <div class="row-content am-cf">
            <ul class="nav-link">
              <li><a href="https://www.baidu.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/baidu.png" /></a></li>
              <li><a href="https://www.google.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/google.png" /></a></li>
              <li><a href="https://www.sina.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/sina.png" /></a></li>
              <li><a href="https://www.aliyun.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/aliyun.png" /></a></li>
              <li><a href="http://www.bootcss.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/bootstrap.png" /></a></li>
              <li><a href="https://github.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/github.png" /></a></li>
              <li><a href="http://www.tmall.com" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/tmall.png" /></a></li>
              <li><a href="http://www.taobao.com" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/taobao.png" /></a></li>
              <li><a href="http://www.jd.com" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/jd.png" /></a></li>
              <li><a href="http://www.iqiyi.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/iqiyi.png" /></a></li>
              <li><a href="https://v.qq.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/vqq.png" /></a></li>
              <li><a href="http://www.youku.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/youku.png" /></a></li>
              <li><a href="http://tv.sohu.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/sohu.png" /></a></li>
              <li><a href="http://www.58.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/58.png" /></a></li>
              <li><a href="http://www.ganji.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/ganji.png" /></a></li>
              <li><a href="https://www.douban.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/douban.png" /></a></li>
              <li><a href="http://www.ctrip.com/" target="_blank"><img class="am-thumbnail" src="assets/img/nav-icon/ctrip.png" /></a></li>
            </ul>
        </div>
        <div class="row-content am-cf">
            <div class="am-g">
              <div data-am-widget="tabs"
                   class="am-tabs am-tabs-d2"
                    >
                  <ul class="am-tabs-nav am-cf">
                      <li class="am-active"><a href="[data-tab-panel-0]">名站导航</a></li>
                      <li class=""><a href="[data-tab-panel-1]">快速通道</a></li>
                  </ul>
                  <div class="am-tabs-bd">
                      <div data-tab-panel-0 class="am-tab-panel am-active">
                        <div class="am-u-sm-12">
                            <section class="am-panel am-panel-default nav-class">
                              <header class="am-panel-hd">
                                <h3 class="am-panel-title"><i class="am-icon-globe"></i> 名站导航</h3>
                              </header>
                              <div class="am-panel-bd">
                                <div class="list">
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[购物]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://www.jumei.com/" target="_blank">聚美优品</a></li>
                                                <li><a href="http://www.vip.com/" class="list-cool" target="_blank">唯品会</a></li>
                                                <li><a href="https://ju.taobao.com/" class="list-cool" target="_blank">聚划算</a></li>
                                                <li><a href="https://www.zhe800.com/" target="_blank">折800</a></li>
                                                <li><a href="http://www.huim.com/" target="_blank">惠喵</a></li>
                                                <li><a href="http://www.yhd.com/" target="_blank">1号店</a></li>
                                                <li><a href="https://www.amazon.cn/" target="_blank">亚马逊</a></li>
                                                <li><a href="https://www.suning.com/" target="_blank">苏宁易购</a></li>
                                                <li><a href="http://www.mogujie.com/" target="_blank">蘑菇街</a></li>
                                                <li><a href="http://www.meilishuo.com/" target="_blank">美丽说</a></li>
                                                <li><a href="http://www.gome.com.cn/" target="_blank">国美在线</a></li>
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[视频]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://www.iqiyi.com/" class="list-cool" target="_blank">爱奇艺</a></li>
                                                <li><a href="http://www.youku.com/" target="_blank">优酷网</a></li>
                                                <li><a href="http://www.le.com/" target="_blank">乐视视频</a></li>
                                                <li><a href="https://v.qq.com/" target="_blank">腾讯视频</a></li>
                                                <li><a href="http://tv.sohu.com/" target="_blank">搜狐视频</a></li>
                                                <li><a href="http://www.mgtv.com/" target="_blank">芒果tv</a></li>
                                                <li><a href="https://www.bilibili.com/" target="_blank">bilibili</a></li>                                  
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[邮箱]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://email.163.com/" target="_blank">163邮箱</a></li>
                                                <li><a href="http://www.126.com/" target="_blank">126邮箱</a></li>
                                                <li><a href="https://mail.qq.com/" target="_blank">QQ邮箱</a></li>
                                                <li><a href="https://mail.aliyun.com/" target="_blank">阿里云邮箱</a></li>
                                                <li><a href="http://mail.sina.com.cn/" target="_blank">新浪邮箱</a></li>
                                                <li><a href="http://www.hotmail.com/" target="_blank">Hotmail</a></li>
                                                <li><a href="http://mail.10086.cn/" target="_blank">139邮箱</a></li>                                  
                                                <li><a href="https://mail.sohu.com/" target="_blank">搜狐邮箱</a></li>                                  
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <div class="clear"></div>
                                </div>
                              </div>
                            </section>
                        </div>
                        <div class="am-u-sm-12">
                            <section class="am-panel am-panel-default nav-class">
                              <header class="am-panel-hd">
                                <h3 class="am-panel-title"><i class="am-icon-legal"></i> 工具网站</h3>
                              </header>
                              <div class="am-panel-bd">
                                <div class="list">
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[工具]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://tu.taobao.com" target="_blank">图片空间</a></li>
                                                <li><a href="http://www.faviconico.org/favicon" target="_blank">ico转换工具</a></li>
                                                <li><a href="http://www.bestcssbuttongenerator.com/" target="_blank">按钮在线生成</a></li>
                                                <li><a href="https://wanwang.aliyun.com/domain/searchresult" target="_blank">域名查询</a></li>
                                                <li><a href="http://cdn.code.baidu.com/?qq-pf-to=pcqq.discussion" class="list-cool" target="_blank">百度静态资源库</a></li>
                                                <li><a href="https://regexper.com/" target="_blank">正则表达式</a></li>
                                                <li><a href="http://tool.lu/css/" target="_blank">代码压缩</a></li>
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[素材]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://www.17sucai.com/" target="_blank">17素材网</a></li>
                                                <li><a href="http://www.58pic.com/" target="_blank">千图网</a></li>
                                                <li><a href="http://888pic.com/" target="_blank">包图网</a></li>
                                                <li><a href="http://huaban.com/" class="list-cool" target="_blank">花瓣网</a></li>
                                                <li><a href="http://codepen.io/#" class="list-cool" target="_blank">codepen</a></li>
                                                <li><a href="http://www.iconfont.cn/" class="list-cool" target="_blank">iconfont</a></li>
                                                <li><a href="https://icomoon.io/" target="_blank">IcoMoon</a></li>                                  
                                                <li><a href="http://www.easyicon.net/" target="_blank">easyicon</a></li>                                  
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <div class="clear"></div>
                                </div>
                              </div>
                            </section>
                        </div>
                        <div class="am-u-sm-12">
                            <section class="am-panel am-panel-default nav-class">
                              <header class="am-panel-hd">
                                <h3 class="am-panel-title"><i class="am-icon-graduation-cap"></i> 学习网站</h3>
                              </header>
                              <div class="am-panel-bd">
                                <div class="list">
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[常用]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="https://github.com/" target="_blank">GitHub</a></li>
                                                <li><a href="http://amazeui.org/" class="list-cool" target="_blank">AmazeUI</a></li>
                                                <li><a href="http://www.bootcss.com/" target="_blank">Bootstrap</a></li>
                                                <li><a href="http://www.layui.com/" target="_blank">layui</a></li>
                                                <li><a href="http://layer.layui.com/" class="list-cool" target="_blank">layer</a></li>
                                                <li><a href="http://www.amazeui-color.com/components.html" target="_blank">AmazeUI-color</a></li>
                                                <li><a href="https://flykeying.github.io/amazeui-ec/index.html" target="_blank">AmazeUI-ec</a></li>
                                                <li><a href="http://t.amazeui.org/" target="_blank">AmazeUI touch</a></li>
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[视频]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://www.imooc.com/" target="_blank">慕课网</a></li>
                                                <li><a href="http://www.jikexueyuan.com/" target="_blank">极客学院</a></li>                              
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <ul class="cf">
                                        <li class="col-1"><a href="http://kan.sogou.com/" target="_blank">[文档]</a></li>
                                        <li class="col-2 a1">
                                            <ul class="cf">
                                                <li><a href="http://cn.vuejs.org/v2/guide/" target="_blank">vue.js</a></li>
                                                <li><a href="https://angular.io/docs/ts/latest/" target="_blank">angular</a></li>
                                                <li><a href="https://mp.weixin.qq.com/debug/wxadoc/dev/?t=20161102" target="_blank">微信小程序</a></li>
                                                <li><a href="http://www.liaoxuefeng.com/" target="_blank">廖雪峰官方网站</a></li>
                                                <li><a href="http://www.w3school.com.cn/" target="_blank">W3school</a></li>   
                                                <li><a href="http://www.layui.com/doc/" target="_blank">layui</a></li>
                                                <li><a href="http://plugins.amazeui.org/" target="_blank">Amaze UI Magnigier</a></li>
                                                <li><a href="http://www.runoob.com/" target="_blank">runoob</a></li>                                                          
                                                <li><a href="http://doc.thinkphp.cn/manual.html" target="_blank">ThinkPHP 3.1</a></li>                                  
                                                <li><a href="http://www.kancloud.cn/thinkphp/thinkphp_quickstart/2140" target="_blank">ThinkPHP 3.2</a></li>
                                                <li><a href="http://www.kancloud.cn/manual/thinkphp5/118003" target="_blank">ThinkPHP 5.0</a></li>                                  
                                            </ul>
                                        </li>
                                        <div class="clear"></div>
                                    </ul>
                                    <div class="clear"></div>
                                </div>
                              </div>
                            </section>
                        </div>
                      </div>
                      <div data-tab-panel-1 class="am-tab-panel ">
                        <div class="am-u-sm-12">
                            <section class="am-panel am-panel-default nav-class">
                              <header class="am-panel-hd">
                                <h3 class="am-panel-title">您最常访问的网址</h3>
                                <div class="setLink setmodal"><i class="am-icon-cog"></i> 设置网址</div>
                              </header>
                              <div class="am-panel-bd">
                                <ul class="setnav-ul">
                                    <li><a href="http://cn.vuejs.org/v2/guide/" target="_blank">vue.js</a></li>
                                    <li><a href="https://angular.io/docs/ts/latest/" target="_blank">angular</a></li>
                                    <li><a href="https://mp.weixin.qq.com/debug/wxadoc/dev/?t=20161102" target="_blank">微信小程序</a></li>
                                    <li><a href="http://www.liaoxuefeng.com/" target="_blank">廖雪峰官方网站</a></li>
                                    <li><a href="http://www.w3school.com.cn/" target="_blank">W3school</a></li>   
                                    <li><a href="http://www.layui.com/doc/" target="_blank">layui</a></li>
                                    <li><a href="http://plugins.amazeui.org/" target="_blank">Amaze UI Magnigier</a></li>
                                    <li><a href="http://www.runoob.com/" target="_blank">runoob</a></li>                                                          
                                    <li><a href="http://doc.thinkphp.cn/manual.html" target="_blank">ThinkPHP 3.1</a></li>                                  
                                    <li><a href="http://www.kancloud.cn/thinkphp/thinkphp_quickstart/2140" target="_blank">ThinkPHP 3.2</a></li>
                                    <li><a href="http://www.kancloud.cn/manual/thinkphp5/118003" target="_blank">ThinkPHP 5.0</a></li>
                                    <li><a href="javascript:;" class="setmodal"><i class="am-icon-plus-circle"></i> 设置网址</a></li>
                                    <div class="clear"></div>                                 
                                </ul>
                              </div>
                            </section>
                        </div> 
                      </div>
                  </div>
              </div>

            </div>
        </div>      
    <?php require_once ("public/footer.php"); ?>

</body>

</html>